...
| Field | Value |
| --- | --- |
| Image digest | `docker inspect --format '{{index .RepoDigests 0}}' agentstrata:<tag>` |
...
7. Build: `docker build -t agentstrata:<commit-short> .` — record the digest.
...
docker sbom agentstrata:<tag> --format cyclonedx-json --output sbom.cdx.json
# Vulnerability scan
trivy image --severity CRITICAL,HIGH --exit-code 1 agentstrata:<tag>
```
