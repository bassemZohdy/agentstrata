...
| 2.9 | E2-5/E2-9: model capability registry (`app/engine/model_catalog.py` — context window, tools, streaming, vision, structured output; LLM-05) defaults `llm.contextWindowTokens` for ENG-04 trimming and rejects boot configs that combine MCP tools with a registry-known non-tool model (unknown models pass by design — a stale registry must not block deployments); `rag.embedding.provider` extended to `azure`/`cohere`/`mistral`/`huggingface`/`watsonx` (E2-9, RAG-01) via the LiteLLM embedding bridge |
| 2.8 | COST-01 extended: the price lookup chain gains the shipped default price catalog (E2-6, `app/engine/pricing.py` keyed by `(provider, model)`, refreshed via `scripts/refresh-pricing.py` with documented provenance) between explicit `costs.models` entries and the flat defaults; with per-agent usage attribution (E2-7, MA-02) each agent's tokens are priced with that agent's effective `(provider, model)` from its deep-merged `llm` block, and the OBS-05 cost counter keeps the root model label |
...
- `llm.provider == "vllm"` (the generic OpenAI-compatible provider, E2-3) ⇒ `llm.baseUrl` set.
- `llm.provider == "azure"` ⇒ `llm.baseUrl` set (the endpoint).
...
| Field | Type | Default | Constraints |
| --- | --- | --- | --- |
| `provider` | enum | `"gemini"` | `gemini \| openai \| anthropic \| ollama \| litellm \| azure \| groq \| mistral \| cohere \| deepseek \| xai \| together \| fireworks \| openrouter \| huggingface \| vllm \| watsonx` (LLM-01a stability) |
...
**LLM-05 (capability registry)** — The runtime ships a curated per-model capability registry (`app/engine/model_catalog.py`: context window, tools, streaming, vision, structured output) used for two purposes: (1) `llm.contextWindowTokens` defaults from the registry when left 0, so ENG-04 history trimming works without manual tuning; (2) boot validation rejects a configuration that combines MCP tools with a registry-known model that does not support tool calling — with the offending path named, exit 78. A model absent from the registry is NEVER rejected (the registry is a reference; a stale table must not block valid deployments). Registry entries are curated with the same manual-refresh policy as the price catalog (E2-6).
...
**LLM-06 (embedding provider set)** — `rag.embedding.provider` supports `gemini | openai | azure | cohere | mistral | huggingface | watsonx` (E2-9, RAG-01): gemini/openai use their native SDKs, the rest go through the LiteLLM embedding bridge with the same model-string prefixes as the LLM set. Drivers fail closed at construction (a missing driver is a ConfigError, never a silent substitute). Bedrock/vertex embeddings remain deferred with the LLM set.
...
error). The lookup chain is: exact `costs.models` entry → the shipped
default price catalog (E2-6, keyed `(provider, model)` in
`app/engine/pricing.py`, refreshable via `scripts/refresh-pricing.py`,
provenance documented in the module) → the flat defaults.  With
per-agent usage attribution (E2-7, MA-02), each agent's tokens are
priced with that agent's effective `(provider, model)` (its optional
`llm` block deep-merged over the root's); without attribution the root
prices the aggregate. When `costs.enabled`, every successful run computes
